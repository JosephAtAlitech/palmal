...
<script type="text/javascript">
	...
	function init() { // Execute after login succeed
		...
		[{type: "type", data: "avl_unit", flags: flags, mode: 0}], 
			...
			function (code) { // updateDataFlags callback
				...
				for (var i = 0; i< units.length; i++){ 
					...
					setTimeout('getSelectedUnitInfo('+u.getId()+','+i+')',1000);
		...
		function getSelectedUnitInfo(unitid,i){ // print information about selected Unit
			...
			if(!unit){ msg("Unit not found");return; } // exit if unit not found
				...
				wialon.util.Gis.getLocations([{lon:pos.x, lat:pos.y}], function(code, address){ 
					if (code) { msg(wialon.core.Errors.getErrorText(code)); return; } // exit if error code
					updateIdleTime(unitid,icon,getName, pos.s,  pos.t,address,i);
				});
		}
		
		function updateIdleTime(unitid,icon,getName,speed,lastmessagetime,address,i)
		{
			//time=time.replace('-','');
			...
			$("#units").append("<tr><td>"+i+"</td><td>"+unitid+"</td><td><img class='icon' src='" + icon + "' alt='icon'/></td><td>" + getName+"</td><td>"+lastmessagetimeDate+"</td><td>"+speed+" Km/h</td><td>"+address+"</td></tr>");
		}	
