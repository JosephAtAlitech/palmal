                ...
                <tbody>
                  <?php
					$sql = "SELECT soldiers_distribute.id,soldiers_distribute.police_id,soldiers_distribute.body_no,soldiers_distribute.cc_number,police_soldier.name,police_soldier.phone,soldiers_distribute.we_product_id,
							...
							WHERE soldiers_distribute.id='".$cName."' AND soldiers_distribute.distribute_dateTime BETWEEN '".$startDate."' AND '".$endtDate."' ORDER BY `soldiers_distribute`.`id` DESC";
					...
					echo "
					<a href='ccNumberWisepdfReports.php?ccID=".$cName."&ccNo=".$fid."&ds=".$startDate."&de=".$endtDate."' target='_blank' title='Issue Details' data-toggle='tooltip' class='btn btn-primary btn-sm btn-flat' style='background: white;color: blue;margin-bottom: 3%;'><i class='fa fa-print'> Print Reports </i></a>
					";
				  ?>
                </tbody>
              </table>
