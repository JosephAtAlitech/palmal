...
class MYPDF extends TCPDF {
    ...
    $pdf->AddPage();
    $content = ''; 
	$sql = "SELECT thana.id,thana.thana_name,thana.address FROM `thana` WHERE thana.id='".$_SESSION['user']."' ";
    ...
    $content .= '<style>p{color:black;font-size: 8px;}
		...
		<table border="" cellspacing="0" cellpadding="3">
			<tr>
				...
				<td width="35%">Delivery Date :<font color="gray"> '.$Ds.'</font> To :<font color="gray"> '.$De.'</font><br>CC Number :<font color="gray"> '.$ccNo.'</font></td>
		...
		$sql12="SELECT soldiers_distribute.id,soldiers_distribute.police_id,soldiers_distribute.body_no,soldiers_distribute.cc_number,police_soldier.name,police_soldier.phone,soldiers_distribute.we_product_id,
					...
					WHERE soldiers_distribute.id='".$cName."' AND soldiers_distribute.re_status='' AND soldiers_distribute.distribute_dateTime BETWEEN '".$Ds."' AND '".$De."' ORDER BY `soldiers_distribute`.`id` DESC";
