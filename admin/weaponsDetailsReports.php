    ...
    $content .= '<style>p{color:black;font-size: 8px;}
		...
		<table border="" cellspacing="0" cellpadding="3">
			...
			<tr>
				<td width="100%">
					...
					<br>Report Generate Date :<font color="gray">  '.date("m-d-Y h:i:s a").' </font>
				</td>
			</tr>
		</table> <br><br>
		...
		<table border="1" cellspacing="0" cellpadding="3">
			...
			$sql = "SELECT product.id,product.type,product.ranks,product.product_image,product.body_no,product.we_name,product.brand_id,product.categories_id,
				SUM(product.quantity) AS quantity,brands.brand_type,brands.brand_name,categories.categories_type,categories.categories_name
				FROM `product` 
				LEFT JOIN brands ON brands.id=product.brand_id
				LEFT JOIN categories ON categories.id=product.categories_id
				where product.type='Weapons'
		...
		<table border="1" cellspacing="0" cellpadding="3">
			...
			$sql = "SELECT product.id,product.type,product.ranks,product.product_image,product.brand_id,product.bu_name,product.categories_id,
					SUM(product.quantity) AS quantity12,brands.brand_type,brands.brand_name,categories.categories_type,categories.categories_name
							FROM `product` 
							LEFT JOIN brands ON brands.id=product.brand_id
							LEFT JOIN categories ON categories.id=product.categories_id
							where product.type='Bullets' GROUP BY categories.categories_name,product.bu_name";
